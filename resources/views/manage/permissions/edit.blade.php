    ...
    <div class="p-4">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
            <div>
                ...
                <p class="text-sm text-zinc-600">Update permission details.</p>
            </div>
            <a href="{{ route('permissions.index') }}" class="inline-flex items-center gap-2 px-3 py-1.5 rounded-md text-sm font-medium bg-white text-zinc-800 border border-zinc-200 hover:bg-zinc-50">Back to list</a>
        </div>
        ...
        <div class="bg-white shadow-sm rounded p-6 border">
            ...
            <form method="POST" action="{{ route('permissions.update', $permission) }}" class="space-y-6">
                ...
                <div class="mb-4 text-sm text-zinc-600">{{ $permission->getKey() }}</div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm">Name</label>
                        <input name="name" value="{{ old('name', $permission->name) }}" class="w-full border rounded px-3 py-2" required />
                    </div>
                    <div>
                        <label class="block text-sm">Guard Name</label>
                        <input name="guard_name" value="{{ old('guard_name', $permission->guard_name) }}" class="w-full border rounded px-3 py-2" />
                    </div>
                </div>
