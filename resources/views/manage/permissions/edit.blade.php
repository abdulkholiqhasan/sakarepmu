    ...
    <div class="p-4">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
            <div>
                ...
                <p class="text-sm text-zinc-600 dark:text-zinc-300">Update permission details.</p>
            </div>
        ...
        <div class="bg-white dark:bg-zinc-900 shadow-sm rounded p-6 border border-zinc-200 dark:border-zinc-700">
            ...
            <form method="POST" action="{{ route('permissions.update', $permission) }}" class="space-y-6">
                ...
                <div class="mb-4 text-sm text-zinc-600 dark:text-zinc-400">{{ $permission->getKey() }}</div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm">Name</label>
                        <input name="name" value="{{ old('name', $permission->name) }}" class="w-full border rounded px-3 py-2 bg-white dark:bg-zinc-800 text-zinc-800 dark:text-zinc-200 border-zinc-200 dark:border-zinc-700" required />
                    </div>
                    <div>
                        <label class="block text-sm">Guard Name</label>
                        <input name="guard_name" value="{{ old('guard_name', $permission->guard_name) }}" class="w-full border rounded px-3 py-2 bg-white dark:bg-zinc-800 text-zinc-800 dark:text-zinc-200 border-zinc-200 dark:border-zinc-700" />
                    </div>
                </div>
