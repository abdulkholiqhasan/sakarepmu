    ...
    <div class="p-4">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
            <div>
                ...
                <p class="text-sm text-zinc-600 dark:text-zinc-300">Manage application users — view, create, and edit accounts.</p>
            </div>
                ...
                <form method="GET" action="{{ route('users.index') }}" class="flex items-center gap-2">
                    ...
                    <input id="users-search" name="q" type="search" placeholder="Search users (press Enter)" value="{{ request('q') }}" class="border rounded-md px-3 py-2 text-sm w-48 bg-white dark:bg-zinc-900 text-zinc-800 dark:text-zinc-200 border-zinc-200 dark:border-zinc-700" aria-label="Search users" />
                </form>
        ...
        <div class="overflow-x-auto bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg shadow">
            <table class="min-w-full divide-y divide-gray-200 rounded-lg text-sm">
                    ...
                    <tr>
                        <th colspan="4" class="px-6 py-4 border-b text-left">
                            <div class="flex items-center justify-between">
                                <div>
                                    <div class="text-sm text-zinc-500 dark:text-zinc-400">Showing</div>
                                    <div class="font-medium text-zinc-900 dark:text-zinc-50">{{ $users->total() }} users</div>
                                </div>
                ...
                <thead class="bg-zinc-50 dark:bg-zinc-800">
                    <tr>
                        <th class="px-6 py-3 text-left text-sm font-medium text-zinc-700 dark:text-zinc-300">User</th>
                        <th class="px-6 py-3 text-left text-sm font-medium text-zinc-700 dark:text-zinc-300">Email</th>
                        <th class="px-6 py-3 text-left text-sm font-medium text-zinc-700 dark:text-zinc-300">Roles</th>
                        ...
                        <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800">
                            <td class="px-6 py-3">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div class="font-medium text-zinc-900 dark:text-zinc-50">{{ $user->name }}</div>
                                        <div class="text-xs text-zinc-500 dark:text-zinc-400">{{ $user->getKey() }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-3 text-zinc-700 dark:text-zinc-200">{{ $user->email }}</td>
                            <td class="px-6 py-3">
                                <div class="flex flex-wrap gap-2">
                                    @forelse($user->roles as $role)
                                        <span class="text-xs bg-indigo-50 dark:bg-zinc-800 text-indigo-700 dark:text-indigo-200 px-2 py-1 rounded">{{ $role->name }}</span>
                                    @empty
