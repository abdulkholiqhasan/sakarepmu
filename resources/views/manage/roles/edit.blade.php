    ...
    <div class="p-4">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
            <div>
                ...
                <p class="text-sm text-zinc-600 dark:text-zinc-300">Modify role details and assigned permissions.</p>
            </div>
            ...
            <form method="POST" action="{{ route('roles.update', $role) }}" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input name="name" value="{{ old('name', $role->name) }}" class="w-full border rounded px-3 py-2 bg-white dark:bg-zinc-800 text-zinc-800 dark:text-zinc-200 border-zinc-200 dark:border-zinc-700" required />
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-zinc-700">Guard Name</label>
                        <input name="guard_name" value="{{ old('guard_name', $role->guard_name) }}" class="w-full border rounded px-3 py-2 bg-white dark:bg-zinc-800 text-zinc-800 dark:text-zinc-200 border-zinc-200 dark:border-zinc-700" />
                    </div>
                </div>
                    ...
                    <div class="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-2">
                        @foreach($permissions as $permission)
                            <label class="flex items-center gap-2">
                                <input type="checkbox" name="permissions[]" value="{{ $permission->getKey() }}" class="h-4 w-4 text-indigo-600 border-gray-300 rounded bg-white dark:bg-zinc-900" {{ $role->permissions->contains($permission) ? 'checked' : '' }} />
                                <span class="text-sm text-zinc-800 dark:text-zinc-200">{{ $permission->name }}</span>
                            </label>
                        @endforeach
                    </div>
                    <p class="text-xs text-zinc-500 dark:text-zinc-400 mt-2">Select permissions granted by this role.</p>
                </div>
