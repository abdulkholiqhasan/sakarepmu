...
<x-layouts.app :title="$title ?? null">
    <div class="bg-white dark:bg-zinc-900 min-h-screen">
        ...
        <div class="bg-white dark:bg-zinc-900 border-b border-gray-200 dark:border-zinc-700 px-4 sm:px-6 py-3 sm:py-4">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-2 sm:space-x-4">
                    <h1 class="text-xl sm:text-2xl font-normal text-gray-900 dark:text-white">Edit Role</h1>
                </div>
        ...
        <form action="{{ route('roles.update', $role) }}" method="POST" class="flex flex-col lg:flex-row">
            @csrf
            @method('PUT')
            <!-- Main content area -->
            <div class="w-full lg:w-3/4 px-4 sm:px-6 py-6">
                ...
                <div class="mb-6 bg-gray-50 dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded-lg p-4">
                    <label class="block text-sm font-medium text-gray-700 dark:text-zinc-300 mb-2">Role ID</label>
                    <div class="text-sm text-gray-600 dark:text-zinc-400 font-mono">{{ $role->getKey() }}</div>
                </div>
                ...
                <div class="mb-6">
                    ...
                    />
                    <p class="text-xs text-gray-500 dark:text-zinc-400 mt-2">The guard defines which authentication system this role applies to.</p>
                    @error('guard_name') 
                        <p class="text-red-600 dark:text-red-400 mt-2 text-sm">{{ $message }}</p> 
                    @enderror
                </div>
            ...
            <div class="w-full lg:w-1/4 bg-gray-50 dark:bg-zinc-800 border-t lg:border-t-0 lg:border-l border-gray-200 dark:border-zinc-700 px-4 py-6 lg:overflow-y-auto">
                ...
                <div class="bg-white dark:bg-zinc-900 border border-gray-300 dark:border-zinc-700 rounded mb-6">
                    <div class="bg-gray-50 dark:bg-zinc-800 px-4 py-3 border-b border-gray-200 dark:border-zinc-700">
                        <h3 class="font-medium text-sm text-gray-900 dark:text-white">Update Role</h3>
                    </div>
                    <div class="p-4">
                ...
                <div class="bg-white dark:bg-zinc-900 border border-gray-300 dark:border-zinc-700 rounded mb-6">
                    ...
                    <div class="p-4 max-h-96 overflow-y-auto">
                        ...
                        <p class="text-xs text-gray-500 dark:text-zinc-400 mt-3">Select permissions granted by this role.</p>
                        @error('permissions') 
                            <p class="text-red-600 dark:text-red-400 mt-2 text-sm">{{ $message }}</p> 
                        @enderror
                    </div>
