...
<x-layouts.app :title="$title ?? null">
    <div class="bg-white dark:bg-zinc-900 min-h-screen">
        ...
        <div class="bg-white dark:bg-zinc-900 border-b border-gray-200 dark:border-zinc-700 px-4 sm:px-6 py-3 sm:py-4">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-2 sm:space-x-4">
                    <h1 class="text-xl sm:text-2xl font-normal text-gray-900 dark:text-white">Edit Tag</h1>
                    @if($tag->name)
                        <span class="text-sm text-gray-500 dark:text-zinc-400">— {{ $tag->name }}</span>
                    @endif
                </div>
        ...
        <form action="{{ route('tags.update', $tag) }}" method="POST" class="flex flex-col lg:flex-row">
            @csrf
            @method('PUT')
