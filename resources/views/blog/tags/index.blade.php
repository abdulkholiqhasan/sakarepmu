...
<x-layouts.app :title="$title ?? null">
    <div class="bg-white dark:bg-zinc-900 min-h-screen">
        ...
        <div class="bg-white dark:bg-zinc-900 border-b border-gray-200 dark:border-zinc-700 px-4 sm:px-6 py-3 sm:py-4">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-2 sm:space-x-4">
                    <h1 class="text-xl sm:text-2xl font-normal text-gray-900 dark:text-white">Tags</h1>
                    <span class="text-sm text-gray-500 dark:text-zinc-400">({{ $tags->total() }})</span>
                </div>
        ...
        <div class="px-4 sm:px-6 py-4">
            ...
            <!-- Search -->
            <div class="mb-4">
                <form method="GET" class="w-full max-w-md" action="{{ route('tags.index') }}">
                    <flux:input name="q" placeholder="Search tags..." type="text" value="{{ request('q') }}" class="w-full" autocomplete="off" enterkeyhint="search" />
                </form>
            </div>
        ...
        <div class="bg-white dark:bg-zinc-800 shadow rounded-lg overflow-hidden">
            @if($tags->isEmpty())
                <div class="p-8 text-center">
                    ...
                    <p class="text-gray-500 dark:text-zinc-400 mb-4">Get started by creating your first tag.</p>
                    @permission('create tags')
                        <button 
                            onclick="window.location.href='{{ route('tags.create') }}'"
                            class="bg-blue-600 hover:bg-blue-700 dark:bg-blue-600 dark:hover:bg-blue-700 text-white font-medium py-2 px-4 rounded text-sm transition-colors touch-manipulation"
                            ...
                            @foreach($tags as $tag)
                                <tr class="hover:bg-gray-50 dark:hover:bg-zinc-700 transition-colors">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-medium text-gray-900 dark:text-white truncate max-w-xs">{{ $tag->name }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-500 dark:text-zinc-400 truncate max-w-sm">{{ $tag->slug }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right">
                                        <div class="flex items-center justify-end space-x-2">
                                            @permission('edit tags')
                                                <button 
                                                    onclick="window.location.href='{{ route('tags.edit', $tag) }}'"
