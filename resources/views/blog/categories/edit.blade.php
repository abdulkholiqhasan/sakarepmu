...
<x-layouts.app :title="$title ?? null">
    <div class="bg-white dark:bg-zinc-900 min-h-screen">
        ...
        <div class="bg-white dark:bg-zinc-900 border-b border-gray-200 dark:border-zinc-700 px-4 sm:px-6 py-3 sm:py-4">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-2 sm:space-x-4">
                    <h1 class="text-xl sm:text-2xl font-normal text-gray-900 dark:text-white">Edit Category</h1>
                    @if($category->name)
                        <span class="text-sm text-gray-500 dark:text-zinc-400">— {{ $category->name }}</span>
                    @endif
                </div>
        ...
        <form action="{{ route('categories.update', $category) }}" method="POST" class="flex flex-col lg:flex-row">
            @csrf
            @method('PUT')
            ...
            <div class="w-full lg:w-1/4 bg-gray-50 dark:bg-zinc-800 border-t lg:border-t-0 lg:border-l border-gray-200 dark:border-zinc-700 px-4 py-6 lg:overflow-y-auto">
                ...
                <div class="bg-white dark:bg-zinc-900 border border-gray-300 dark:border-zinc-700 rounded mb-6">
                    <div class="bg-gray-50 dark:bg-zinc-800 px-4 py-3 border-b border-gray-200 dark:border-zinc-700">
                        <h3 class="font-medium text-sm text-gray-900 dark:text-white">Update</h3>
                    </div>
                    <div class="p-4">
...
<script>
    // Auto-generate slug from name (Admin style) - For edit page, assume initially edited
    (function(){
        const name = document.querySelector('input[name="name"]');
        const slug = document.querySelector('input[name="slug"]');
        if(!name || !slug) return;
        let manuallyEdited = true; // Start as true for edit pages
