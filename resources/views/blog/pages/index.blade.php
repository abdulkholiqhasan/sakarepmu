        ...
        <div class="bg-white dark:bg-zinc-900 border-b border-gray-200 dark:border-zinc-700 px-4 sm:px-6 py-3 sm:py-4">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-500 dark:text-zinc-400">Manage your static pages</p>
                </div>
                <div>
                    @permission('manage pages')
                        <button 
                            onclick="window.location.href='{{ route('pages.create') }}'"
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            ...
            <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <div class="flex items-center space-x-1 text-sm">
                    ...
                    <a href="{{ route('pages.index', ['status' => 'published']) }}" 
                    ...
                    <a href="{{ route('pages.index', ['status' => 'draft']) }}" 
                    ...
                    <div class="text-center py-12">
                        <div class="mx-auto h-12 w-12 text-gray-400 dark:text-zinc-500 mb-4">
                            <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m6.75 18.75H8.25A2.25 2.25 0 016 16.5V4.25a2.25 2.25 0 012.25-2.25h5.25a2.25 2.25 0 012.25 2.25v12.5a1.125 1.125 0 01-2.25 0v-8.5a.375.375 0 00-.375-.375H10.5" />
                            </svg>
                        </div>
                                    ...
                                    <tr class="hover:bg-gray-50 dark:hover:bg-zinc-700/50 transition-colors">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-500 dark:text-zinc-400">
                                                <p class="font-medium">{{ $page->published_at ? $page->published_at->format('M d, Y') : '—' }}</p>
                                                <p class="text-xs">{{ $page->published_at ? $page->published_at->format('g:i A') : '' }}</p>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-500 dark:text-zinc-400">
                                                <p class="font-medium">{{ $page->updated_at->format('M d, Y') }}</p>
                                                <p class="text-xs">{{ $page->updated_at->format('g:i A') }}</p>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right">
                                            <div class="flex items-center justify-end space-x-2">
                                                @permission('manage pages|edit pages')
                                                    <button 
                                                        onclick="window.location.href='{{ route('pages.edit', $page) }}'"
                                                ...
                                                @endpermission
                                                @permission('manage pages|delete pages')
                    ...
                    <div class="bg-gray-50 dark:bg-zinc-700/30 px-6 py-4 border-t border-gray-200 dark:border-zinc-700">
                        <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                            <div class="flex items-center text-sm text-gray-500 dark:text-zinc-400">
                                ...
                                <span class="font-medium text-gray-900 dark:text-white">{{ $pages->firstItem() ?? 0 }}</span>
                                <span class="mx-1">to</span>
                                <span class="font-medium text-gray-900 dark:text-white">{{ $pages->lastItem() ?? 0 }}</span>
                                <span class="mx-1">of</span>
                                <span class="font-medium text-gray-900 dark:text-white">{{ $pages->total() }}</span>
